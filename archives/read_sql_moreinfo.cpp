...
bool compare(duckdb::MaterializedQueryResult& duckdb_results, const ColumnarTable& results) {
    ...
    if (duckdb_results.RowCount() != num_rows) {
        fmt::println(stderr, "  Row count mismatch: DuckDB={}, result={}", duckdb_results.RowCount(), num_rows);
        return false;
    }
    if (duckdb_results.ColumnCount() != num_cols) {
        fmt::println(stderr, "  Column count mismatch: DuckDB={}, result={}", duckdb_results.ColumnCount(), num_cols);
        return false;
    }
    for (size_t i = 0; i < num_cols; ++i) {
        if (not equal(duckdb_results.types[i], results.columns[i].type)) {
            fmt::println(stderr, "  Column type mismatch at column {}", i);
            return false;
        }
    }
    ...
    if (mismatch_count > 0) {
        fmt::println(stderr, "  Row data mismatch: {} of {} rows differ (first at row {})", mismatch_count, num_rows, first_mismatch);
        
        // Show first mismatch details
        if (first_mismatch < num_rows) {
            fmt::println(stderr, "  First mismatch at row {}:", first_mismatch);
            const auto& expected_row = duckdb_table[first_mismatch];
            const auto& actual_row = result_table.table()[first_mismatch];
            
            for (size_t col = 0; col < std::min(num_cols, size_t(10)); ++col) {
                bool col_matches = expected_row[col] == actual_row[col];
                if (!col_matches) {
                    ...
                    std::visit([&](auto&& val) {
                        ...
                        if constexpr (std::is_same_v<T, std::monostate>) {
                            ...
                            act_str = fmt::format("{}", val);
                        }
                    }, actual_row[col]);
                    
                    // For strings, show length and find first diff
                    if (exp_str != "NULL" && act_str != "NULL" && 
                        (exp_str.length() > 100 || act_str.length() > 100)) {
                        size_t diff_pos = 0;
                        size_t min_len = std::min(exp_str.length(), act_str.length());
                        while (diff_pos < min_len && exp_str[diff_pos] == act_str[diff_pos]) {
                            diff_pos++;
                        }
                        fmt::println(stderr, "    col[{}]: string mismatch (exp_len={}, got_len={})", 
                                   col, exp_str.length(), act_str.length());
                        if (diff_pos < min_len) {
                            size_t ctx_start = diff_pos > 40 ? diff_pos - 40 : 0;
                            size_t ctx_end = std::min(diff_pos + 60, min_len);
                            fmt::println(stderr, "      First diff at pos {}", diff_pos);
                            fmt::println(stderr, "      Expected[{}:{}]: ...{}...", 
                                       ctx_start, ctx_end, exp_str.substr(ctx_start, ctx_end - ctx_start));
                            fmt::println(stderr, "      Got[{}:{}]:      ...{}...", 
                                       ctx_start, ctx_end, act_str.substr(ctx_start, ctx_end - ctx_start));
                        } else {
                            fmt::println(stderr, "      Strings match up to pos {} but differ in length", diff_pos);
                        }
                    } else {
                        fmt::println(stderr, "    col[{}]: expected={}, got={}", col, exp_str, act_str);
                    }
                }
            }
        }
...
bool compare_withcache(const ColumnarTable& expected, const ColumnarTable& results) {
    ...
    if (mismatch_count > 0) {
        ...
        if (first_mismatch < num_rows) {
            fmt::println(stderr, "  First mismatch at row {}:", first_mismatch);
            const auto& expected_row = expected_table.table()[first_mismatch];
            const auto& actual_row = results_table.table()[first_mismatch];
